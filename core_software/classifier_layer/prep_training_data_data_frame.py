...
class AugmentedDataGenerator:
    ...
    def __init__(self,data_dir,out_target='../../data/full_augmented_data_seperate',augment_multiplier=10):
    ...
    def _iterate_file_list(self):
        ...
        MSGs = []
        full_labels = []
        data_frame=pd.DataFrame(columns=['id','tone','phoneme'])


        sample_counter=0
        for file in tqdm(self.files):

            if self.augment_multiplyer==1:
                masked = mp3toMSG(file, trimming=True)
                ...
                padded_data = add_padding(np.expand_dims(masked, -1), bonus_padding=2, maxes=self.data_max_dims)[0]
                title = f'sample_{sample_counter}.npy'

                fragment = file.split('/')[-1].split('_')[0]
                np.save(os.path.join(self.out_target, title), padded_data)
                data_frame = data_frame.append({'id': title, 'tone': fragment[-1], 'phoneme': fragment[:-1]},
                                               ignore_index=True)
                sample_counter += 1
                ...
                for n in range(self.augment_multiplyer):
                    ...
                    fragment=file.split('/')[-1].split('_')[0]
                    np.save(os.path.join(self.out_target,title), padded_data)
                    data_frame=data_frame.append({'id':title,'tone':fragment[-1],'phoneme':fragment[:-1]},ignore_index=True)
                    sample_counter+=1

        data_frame.to_csv(os.path.join(self.out_target,'mapping.csv'))
